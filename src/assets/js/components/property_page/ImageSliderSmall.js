...
const ImageSliderSmall = (props) => {
  ...
  const mobile =
    window.matchMedia(
      '(min-width: 768px) and (max-height: 1024px) and (orientation: portrait)',
    ).matches ||
  ...
  const displayImages = () => {
    document.querySelectorAll('.review-image').forEach((image, index) => {
      ...
      if (shownImage >= document.querySelectorAll('.review-image').length - 1)
        image_pos = image_width * (index - 2);
  ...
  const changeImage = (e) => {
    ...
    if (classList.contains('image-small') && toggle_event) {
      ...
      document.querySelector('.image-showcase > p span').textContent =
        current_image + 1;
      ...
      document.querySelectorAll('.image-small').forEach((image) => {
        ...
        setTimeout(
          () => image.setAttribute('data-toggle-event', 'true'),
          transitionTime,
        );
      });
      ...
      document.querySelectorAll('.review-image').forEach((image, index) => {
        ...
        if (index === current_image)
          image.style.transform = `${direction}(${0}px)`;
