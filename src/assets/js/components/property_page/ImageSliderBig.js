...
const ImageSliderBig = (props) => {
  ...
  const displayImages = () => {
    document
      .querySelectorAll('#images-slider-big .image-gallery > div')
      .forEach((image, index) => {
        image.style.transition = 'none';
        setTimeout(() => (image.style.transition = ''), 100);

        const image_margin = parseFloat(
          window.getComputedStyle(image).getPropertyValue('margin-left'),
        );
        const image_width = Math.ceil(
          image.getBoundingClientRect().width + image_margin,
        );
  ...
  const changeImage = (e) => {
    ...
    if (target.tagName === 'DIV') {
      ...
      if (target.id.includes('right')) {
        if (count === 0)
          count =
            document.querySelectorAll('#images-slider-big .image-gallery > div')
              .length - 1;
        else count--;
      }
  ...
  const toggleSlider = (e) => {
    if (
      e.target.id === 'images-slider-big' ||
      e.currentTarget.id === 'close-slider'
    ) {
      toggleSliderModal(false);

      let image =
        parseFloat(
          document.querySelector('.image-gallery > p span').textContent,
        ) - 1;
  ...
  return (
    ...
    >
      ...
      <div className='image-gallery mx-5 position-relative overflow-hidden'>
        {images.map((image) => (
          <div key={image} className='position-absolute mx-5'>
            <Image src={image} />
          </div>
        ))}
