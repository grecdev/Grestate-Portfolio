...
export class AuthenticationContextProvider extends Component {
  ...
  signUpAuth(signup_state) {
    ...
    firebase_auth
      ...
      .catch((err) => {
        err &&
          this.setState({
            authentication_regex: err.message,
            auth_loader: false,
          });
      });
  }
  ...
  signOutAuth() {
    ...
    const { location, changePage } = this.context;
  ...
  componentDidUpdate(prevState) {
    if (
      prevState.login_enabled !== this.state.login_enabled ||
      prevState.signup_enabled !== this.state.signup_enabled ||
      prevState.reset_password_enabled !== this.state.reset_password_enabled
    ) {
      if (
        this.state.login_enabled ||
